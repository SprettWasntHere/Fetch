...
def update_ytdlp(log_callback):
    try:
        ...
        log_callback("Error: yt-dlp is missing from the environment.")
        return None
...
def run_download(
    ...
    audio_format = (
        ...
        else ("wav" if "WAV" in format_choice else None)
    )
    ...
    try:
        ...
        is_playlist = (
            ...
            or bool(info.get("playlist_title") or info.get("album"))
        )
        ...
        def download_single_track(track_url):
            ...
            if audio_format:
                fallback_album = (
                    playlist_name
                    if playlist_name != "Single"
                    else os.path.basename(target_dir) or "Single"
                )
                ...
                if embed_cover:
                    ...
                    track_opts["outtmpl"] = {
                        ...
                        "thumbnail": os.path.join(
                            target_dir, "%(title)s [%(id)s]"
                        ),
                    }
                    track_opts["postprocessor_args"].extend(
                        ["-id3v2_version", "3", "-write_id3v1", "1"]
                    )
                    track_opts["postprocessors"].append(
                        {"key": "EmbedThumbnail"}
                    )
            else:
                track_opts["format"] = (
            ...
            def ydl_hook(d):
                ...
                if d["status"] == "downloading":
                    ...
                    if total_bytes and progress_callback:
                        ...
                        base_progress = (
                            completed_tracks[0] / total_tracks
                        ) * 100
                    ...
                    log_callback(
                        f"[{timestamp}] Finished processing: {os.path.basename(d.get('filename', 'file'))}"
                    )
            ...
            try:
                ...
                if progress_callback:
                    progress_callback(
                        (completed_tracks[0] / total_tracks) * 100
                    )
        ...
        for i in range(0, len(track_urls), batch_size):
            ...
            if pause_duration > 0 and i + batch_size < len(track_urls):
                timestamp = time.strftime("%H:%M:%S")
                log_callback(
                    f"[{timestamp}] Downloaded {batch_size} tracks. Pausing for {pause_duration} seconds to avoid rate-limiting..."
                )
